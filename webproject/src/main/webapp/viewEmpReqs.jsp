...
<body>
	...
	try {
		...
		PreparedStatement ps = con.prepareStatement("select username from people where id=?");
		ps.setString(1, id);
		...
		<thead>
      	...
      	<tbody>
        <%
        ps = con.prepareStatement("select * from reimb where author=?");
        ps.setString(1, id);
        rs = ps.executeQuery();
        while(rs.next()) {
            %>
	...
	<%}
			...
			out.print("Please input a correct Employee ID.<br></br>");
		}
	} catch (Exception e) {
		e.printStackTrace();
	}
